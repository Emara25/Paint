...
export default {
  ...
  methods: {
    ...
    handleButtonClick(button) {
      ...
      if (value === "rectangle") {
        ...
        const savePath = 'Paint.json';
        axios.post(`http://localhost:8080/save/all/json?path=${savePath}`)
        .then(response => {
    ...
    loadFile() {
        ...
        if (filePath) {
          const url = `http://localhost:8080/load/json?path=${filePath}`;
  ...
  showErrorMessage(message) {
    ...
    copySelectedShape() {
      ...
      if(this.selectedShape != null){
        var clonedShape;
          ...
          else if(this.selectedShape.attrs.type=='rect'){
    ...
    axiosPost(shape){
      axios.post(`http://localhost:8080/save/${shape.attrs.type}`, this.dummyBackObj)
        .then(response => {
    ...
    undo(){
      ...
      .then(response => {
       ...
       console.log('loaded from back :' ,loaded);
        console.log('existing:' , existing); 
        console.log("caompare " + this.compareExisitingShape(loaded));
       if(this.compareExisitingShape(loaded)){
          this.selectedShape = loaded;
          this.deleteSelectedShape();
        }else{
          this.updateExisitingShape(loaded);
        }

        console.log('Shape loaded successfully:', response.data);  
      }).catch(error => {
 ...
 createDummyBackObj(shape){
  ...
  ,compareExisitingShape(loaded) {
  var existingShape = this.getShapeById(loaded.id);
  this.createDummyBackObj(existingShape);

  // Ensure that the type, id, and color match
  if (
    
    this.dummyBackObj.color === loaded.color &&
    this.dummyBackObj.properties.length === loaded.properties.length
  ) {
    // Compare each property individually
   
      if (JSON.stringify(this.dummyBackObj.properties) !== JSON.stringify(loaded.properties)) {
  ...
  updateExisitingShape(loaded){
    var existingShape = this.getShapeById(loaded.id);
    existingShape.fill(loaded.color);
    existingShape.rotation(loaded.properties[0]);
    existingShape.x(loaded.properties[1]);
    existingShape.y(loaded.properties[2]);
    switch(loaded.type){
        case 'triangle':
        case 'circle':
          existingShape.radius(loaded.properties[3]);
          break;
        case 'line':
        case 'square':
        case 'rect':
          existingShape.width ( loaded.properties[3]) ; 
          existingShape.height ( loaded.properties[4] );
          break;
        case 'ellipse':
          existingShape.radiusX ( loaded.properties[3]);
          existingShape.radiusY ( loaded.properties[4]);
          break; 
    }
    this.layer.batchDraw();
